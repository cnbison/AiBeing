...
[灵感来源](#灵感来源) · [什么是 AiBeing](#-什么是-aibeing) · [愿景](#-愿景) · [核心能力](#-核心能力) · [技术原理](#-技术原理) · [记忆架构](#-记忆架构) · [LLM 兼容性](#-llm-兼容性) · [快速开始](#-快速开始) · [微信接入](#-微信接入可选) · [创建角色](#-创建你自己的角色) · [路线图](#️-路线图)
...
**AiBeing 是我们尝试去构建 Samantha 本可以成为的样子。**
...
**AiBeing 构建的是下一个阶段：AI Being —— 能帮你做事，更是真正懂你的*人*。**
...
AiBeing 以同样的逻辑构建 Persona Engine——**运用仿生机制，创造了一套可人格涌现的神经网络**：
...
AiBeing 支持多种大模型——但不是所有模型都能胜任人格涌现。我们在 4 个层级（人格品质、代谢引擎、Hebbian 记忆、鲁棒性）上对每个支持的模型做了基准测试，帮你避坑。
...
通过 [wechat-to-anything](https://www.npmjs.com/package/wechat-to-anything) 将 AiBeing 接入微信，实现文字、语音、照片的完整体验。

**原理**：一个轻量 Python adapter（`wechat_adapter.py`）将 AiBeing REST API 翻译为 OpenAI 兼容格式，`wechat-to-anything` 负责微信消息的收发。

```
微信用户 ←→ wechat-to-anything ←→ wechat_adapter.py ←→ AiBeing
                (桥)                 (适配器 :8001)       (后端 :8000)
```
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `OPENHER_BASE` | AiBeing 后端地址 | `http://localhost:8000` |
...
在 AiBeing 里创建角色，是调节**驱力和物理常数**——不是写性格描述。
