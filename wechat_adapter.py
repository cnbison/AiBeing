"""
微信 ↔ AiBeing 适配层（持久 WebSocket 版）

与桌面端完全一致的连接模式：
  - 启动时建立一个持久 WebSocket 到 AiBeing /ws/chat
  - 所有聊天消息通过此连接收发
...
class PersistentWS:
    """
    管理一个持久 WebSocket 连接到 AiBeing /ws/chat。
...
if __name__ == "__main__":
    print(f"🔗 AiBeing WeChat Adapter (持久 WebSocket)")
    print(f"   AiBeing: {OPENHER_BASE}")
